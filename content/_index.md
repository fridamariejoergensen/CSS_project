...
We will use web scraping to get a list of all names from Wikipedia. To create the network, we will also use web scraping where Nodes are individuals, and edges are found from links in the text on each person's page (if the link is also a person) on the Harry Potter Wiki. To populate the nodes with attributes we will use a Harry Potter database API. We will perform text processing on the content of the specific pages for individuals on the Harry Potter Wiki (through the Harry Potter Wiki API).
